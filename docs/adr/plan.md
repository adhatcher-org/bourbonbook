...
| A23 | Fill level drives bottle status on the edit form | In Progress | `codex/fill-level-drives-status` | [PR #84](https://github.com/adhatcher-org/bourbonbook/pull/84) (draft) at `2808be6`. Planned 2026-09-22; `architecture-critic` returned **APPROVE** in round 2, and Aaron approved the plan on 2026-09-22. Decisions: keep A23; choosing a status moves the slider and Opened restores the last level (1d); client-only, with no server enforcement and no ADR (A); a polite status live region, with a pre-authorized fill-level fallback (B, B+); gaps 1–2 get their own actions later; the focus fix stays in scope. The work is client-side only: the slider and status sync, the live region, the status-picker focus rule, and `CACHE` v8. No route, schema, or migration change. Evidence: `bourbonbook-reviewer` **PASS** and `pr-validator` **PASS** at `2808be6`; `make pr-review` exit 0 (ruff, 422 tests, 90.54% branch coverage against an 80% floor, bandit, pip-audit, frozen `uv lock`, `scripts/pr_review.py` 10 passed, container build `bourbonbook:local-v1`); `vux-tester` J1–J13 all pass at 390×844 and 1280×800 with zero console errors and zero axe-core violations. **The PR stays in draft until Aaron’s VoiceOver pass (AC15) is recorded**; the B+ fill-level-announcement fallback is pre-authorized if VoiceOver does not speak the slider move. Pre-existing gap 1 (`/analyze` has no became-Empty guard) remains deferred under D4 and is untouched, though marginally easier to reach. The ID skips A18–A22 deliberately; the section explains why. **Stays `In Progress` until [PR #84](https://github.com/adhatcher-org/bourbonbook/pull/84) merges**: the PR is an unmerged draft and AC15 is outstanding, so `Complete` would overstate the state regardless of lifecycle step 12. |
...
**Required manual check.** Aaron runs this in person, because a person has to hear the speech. Use macOS Safari with VoiceOver (Cmd+F5) against the throwaway instance, on B1's edit page.
