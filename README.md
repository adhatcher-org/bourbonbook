...
Open `http://localhost:8000`, create an account, and add a bottle. Local development uses `https://ollama.aaronhatcher.com` by default. If Ollama is not reachable, the photo is still saved and the review form opens for manual entry.
...
Evaluate a local Ollama model against the bottle-image fixtures:

```bash
uv run --env-file .env python -m scripts.evaluate_ollama --model gemma3:4b
```
